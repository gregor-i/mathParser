...
object Parser {
  ...
  def parse[UO, BO, S, V](lang: Language[UO, BO, S, V], literalParser: LiteralParser[S])(input: String): Option[Node[UO, BO, S, V]] = {
    ...
    def constant(input: String): Option[Node[UO, BO, S, V]] =
      lang.constants.find(_._1.name == input).map(constant => ConstantNode[UO, BO, S, V](constant._2))

    def variable(input: String): Option[Node[UO, BO, S, V]] = lang.variables.find(_._1.name == input).map(v => VariableNode(v._2))
    ...
    def binaryInfixOperation(input: String): Option[Node[UO, BO, S, V]] = lang.binaryInfixOperators
      .flatMap(op => binaryNode(input, op._1.name.head, BinaryNode[UO, BO, S, V](op._2, _, _)))
      .headOption

    def binaryPrefixOperation(input: String): Option[Node[UO, BO, S, V]] = lang.binaryPrefixOperators
      .find(op => input.matches(s"${op._1.name}\\(.*\\)"))
      .flatMap(op => binaryNode(input.substring(op._1.name.length + 1, input.length - 1), ',', BinaryNode[UO, BO, S, V](op._2, _, _)))

    def unitaryPrefixOperation(input: String): Option[Node[UO, BO, S, V]] = lang.unitaryOperators
      .find {
        case op if !op._1.name.exists(_.isLetterOrDigit) => input.startsWith(op._1.name)
        case op => input.startsWith(op._1.name + "(") || input.startsWith(op._1.name + " ")
      }
      .flatMap(op => loop(input.stripPrefix(op._1.name)).map(UnitaryNode[UO, BO, S, V](op._2, _)))
